...
public class BitbucketServerClient {
	...
	public BitbucketServerClient(final ViolationsLogger violationsLogger,
			...
			final String proxyHostNameOrIp, final Integer proxyHostPort,
			final String proxyUser, final String proxyPassword) {
		this.violationsLogger = violationsLogger;
		if (bitbucketServerBaseUrl.endsWith("/")) {
			this.bitbucketServerBaseUrl = bitbucketServerBaseUrl.substring(0,
					bitbucketServerBaseUrl.length() - 1);
		} else {
		...
		this.proxyInformation = new ProxyConfig(proxyHostNameOrIp,
				proxyHostPort, proxyUser, proxyPassword);
	}
	...
	private String getBitbucketServerPullRequestBase() {
		return this.getBitbucketServerApiBase() + "/projects/"
				+ this.bitbucketServerProject + "/repos/"
				+ this.bitbucketServerRepo + "/pull-requests/"
				+ this.bitbucketServerPullRequestId;
	}

	private <T> T invokeAndParse(final String url, final Method method,
			final String postContent, final String jsonPath) {
		final String json = this.doInvokeUrl(url, method, postContent);

		try {
			return JsonPath.read(json, jsonPath);
		} catch (final Exception e) {
			throw new RuntimeException("Unable to parse diff response from "
					+ url + " using " + jsonPath + "\n\n" + json, e);
		}
	}

	public List<String> pullRequestChanges() {
		final String url = this.getBitbucketServerPullRequestBase()
				+ "/changes?limit=999999";
		...
		try {
			...
			if (response.isEmpty()) {
				this.violationsLogger.log(INFO, "Found no changed files from "
						+ url + " with JSONPath " + jsonPath + " in JSON:\n"
						+ json);
			}
			return response;
		} catch (final Exception e) {
	...
	public void pullRequestComment(final String message) {
		...
		this.doInvokeUrl(
				...
				BitbucketServerInvoker.Method.POST, postContent);
	}

	private String doInvokeUrl(final String url, final Method method,
			final String postContent) {
		if (isNullOrEmpty(this.bitbucketServerUser)
				|| isNullOrEmpty(this.bitbucketServerPassword)) {
			return bitbucketServerInvoker.invokeUrl(this.violationsLogger, url,
					method, postContent, this.bitbucketPersonalAccessToken,
					this.proxyInformation);
		} else {
			return bitbucketServerInvoker.invokeUrl(this.violationsLogger, url,
					method, postContent, this.bitbucketServerUser,
					this.bitbucketServerPassword, this.proxyInformation);
		}
	}

	public BitbucketServerComment pullRequestComment(final String changedFile,
			int line, final String message) {
		if (line == 0) {
			line = 1;
		}
		final String commentPostContent = "{ \"text\": \""
		...
		final LinkedHashMap<?, ?> parsed = this.invokeAndParse(
				this.getBitbucketServerPullRequestBase() + "/comments",
				BitbucketServerInvoker.Method.POST, commentPostContent, "$");
	...
	public BitbucketServerComment pullRequestComment(final Long commentId) {
		...
		final LinkedHashMap<?, ?> parsed = this.invokeAndParse(url,
				BitbucketServerInvoker.Method.GET, null, "$");
	...
	public List<BitbucketServerComment> pullRequestComments() {
		final String url = this.getBitbucketServerPullRequestBase()
				+ "/activities?limit=9999";
		final List<BitbucketServerComment> comments = this.getComments(url,
				"$.values.[*].comment");
		return comments;
	}
	...
	public List<BitbucketServerComment> pullRequestComments(
		...
		try {
			...
			final String url = this.getBitbucketServerPullRequestBase()
					+ "/comments?path=" + encodedChangedFile
	...
	private List<BitbucketServerComment> getComments(final String url,
		...
		if (parsed.isEmpty()) {
			this.violationsLogger.log(INFO, "Found no comments from " + url
					+ " with JSONPath " + jsonPath + " in JSON:\n" + json);
		}
	...
	public BitbucketServerDiffResponse pullRequestDiff(final String path) {
		final String url = this.getBitbucketServerPullRequestBase() + "/diff/"
				+ path;
		final String json = this.doInvokeUrl(url,
				BitbucketServerInvoker.Method.GET, null);
		try {
			final BitbucketServerDiffResponse diff = new Gson().fromJson(json,
					BitbucketServerDiffResponse.class);
			if (diff.getDiffs().isEmpty()) {
				this.violationsLogger.log(INFO, "Found no diffs from " + url
						+ " in JSON:\n" + json);
			}
			return diff;
		} catch (final Exception e) {
	...
	public void pullRequestRemoveComment(final Integer commentId,
			final Integer commentVersion) {
		this.doInvokeUrl(this.getBitbucketServerPullRequestBase()
				+ "/comments/" + commentId + "?version=" + commentVersion,
				BitbucketServerInvoker.Method.DELETE, null);
	}
	...
	public void commentCreateTask(final BitbucketServerComment comment,
			final String changedFile, final int line) {
		final String changedFileName = new File(changedFile).getName();

		final String taskPostContent = "{ \"anchor\": { \"id\": "
				+ comment.getId()
				+ ", \"type\": \"COMMENT\" }, \"text\": \"[Violation] "
				+ changedFileName + " L" + line + "\" }}";

		this.doInvokeUrl(this.getBitbucketServerApiBase() + "/tasks",
				Method.POST, taskPostContent);
	}
	...
	private BitbucketServerComment toBitbucketServerComment(
		...
		if (jsonArrayTasks != null) {
			tasks = Arrays.asList(jsonArrayTasks
					.toArray(new LinkedHashMap<?, ?>[0]));
		}

		List<LinkedHashMap<?, ?>> subComments = new ArrayList<>();
		final JSONArray jsonArraySubComments = (JSONArray) parsed
				.get("comments");
		if (jsonArraySubComments != null) {
			subComments = Arrays.asList(jsonArraySubComments
					.toArray(new LinkedHashMap<?, ?>[0]));
		}

		final List<BitbucketServerTask> bitbucketServerTasks = this
				.toBitbucketServerTasks(tasks);
		final List<BitbucketServerComment> bitbucketServerSubComments = this
				.toBitbucketServerComments(subComments);

		final BitbucketServerComment comment = new BitbucketServerComment(
				version, text, id);
	...
	private List<BitbucketServerTask> toBitbucketServerTasks(
			final List<LinkedHashMap<?, ?>> parsed) {
	...
	private BitbucketServerTask toBitbucketServerTask(
			final LinkedHashMap<?, ?> parsed) {
		final Integer id = (Integer) parsed.get("id");
		final String text = (String) parsed.get("text");
